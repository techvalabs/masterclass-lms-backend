...
class PaymentService {
      ...
      if (paymentIntent.status === 'succeeded') {
        ...
        const transactions = await db.query<RowDataPacket>(
          `SELECT pt.*, c.title as course_title
           ...
           WHERE pt.stripe_payment_intent_id = ?`,
          [paymentIntentId]
        );
  ...
  async processRefund(transactionId: string, amount?: number, reason?: string) {
    try {
      ...
      await db.query(
        ...
        [refundAmount, reason || 'Customer requested', transactionId]
      );
  ...
  async validateCoupon(code: string, courseId?: number): Promise<Coupon | null> {
    try {
      let query = `
        SELECT * FROM coupons 
        WHERE code = ? 
  ...
  async applyCoupon(transactionId: string, couponCode: string) {
    try {
      ...
      await db.query(
        `UPDATE payment_transactions 
         SET coupon_code = ?,
             ...
             net_amount = ?
         WHERE transaction_id = ?`,
        [couponCode, discountAmount, netAmount, transactionId]
      );
