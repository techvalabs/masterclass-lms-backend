...
interface NotificationData {
  userId: number;
  type: 'course_enrollment' | 'course_completion' | 'new_review' | 'new_message' | 
        'assignment_graded' | 'course_update' | 'payment_received' | 'certificate_earned' |
        'quiz_completed' | 'forum_reply' | 'instructor_announcement' | 'system_alert';
  title: string;
  message: string;
...
interface StoredNotification extends RowDataPacket {
  id: number;
  user_id: number;
  type: string;
  title: string;
...
class NotificationService {
  ...
  private setupSocketHandlers() {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('mark-as-read', async (notificationId: number) => {
      ...
      socket.on('fetch-notifications', async (options: { limit?: number; offset?: number }) => {
  ...
  async sendNotification(data: NotificationData): Promise<void> {
    try {
      ...
      const [result] = await pool.execute<ResultSetHeader>(
        `INSERT INTO notifications (user_id, type, title, message, metadata, action_url, priority)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
          data.userId,
          data.type,
  ...
  private async getUserNotifications(
    ...
    try {
      ...
      const [notifications] = await pool.execute<StoredNotification[]>(
  ...
  private async markAsRead(userId: number, notificationId: number): Promise<void> {
    try {
      const pool = db.getPool();
      if (!pool) return;
  ...
  private async markAllAsRead(userId: number): Promise<void> {
    try {
      const pool = db.getPool();
      if (!pool) return;
  ...
  private async sendExternalNotifications(data: NotificationData): Promise<void> {
    try {
      const pool = db.getPool();
      if (!pool) return;
  ...
  async notifyCourseEnrollment(userId: number, courseId: number, courseTitle: string) {
  ...
  async notifyCourseCompletion(userId: number, courseId: number, courseTitle: string) {
    await this.sendNotification({
      userId,
  ...
  async notifyNewReview(instructorId: number, courseTitle: string, rating: number) {
    await this.sendNotification({
      userId: instructorId,
      type: 'new_review',
  ...
  async notifyPaymentReceived(userId: number, amount: number, courseTitle: string) {
    await this.sendNotification({
      userId,
  ...
  async notifyAssignmentGraded(userId: number, assignmentTitle: string, grade: string) {
    await this.sendNotification({
      userId,
  ...
  async notifyInstructorAnnouncement(courseId: number, announcement: string) {
    const pool = db.getPool();
    if (!pool) return;
