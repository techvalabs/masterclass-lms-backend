...
router.post('/images',
  upload.single('image'),
  validateFileUpload(['jpg', 'jpeg', 'png', 'gif', 'webp'], 5 * 1024 * 1024), // 5MB limit
...
router.post('/images/multiple',
  ...
  validateFileUpload(['jpg', 'jpeg', 'png', 'gif', 'webp'], 5 * 1024 * 1024),
...
router.post('/videos',
  ...
  upload.single('video'),
  validateFileUpload(['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm'], 500 * 1024 * 1024), // 500MB limit
...
router.post('/documents',
  ...
  validateFileUpload(['pdf', 'doc', 'docx', 'txt', 'rtf'], 50 * 1024 * 1024), // 50MB limit
...
router.post('/documents/multiple',
  ...
  validateFileUpload(['pdf', 'doc', 'docx', 'txt', 'rtf'], 50 * 1024 * 1024),
...
router.post('/audio',
  upload.single('audio'),
  validateFileUpload(['mp3', 'wav', 'ogg', 'm4a'], 100 * 1024 * 1024), // 100MB limit
...
router.post('/videos/:fileId/process',
  requireInstructorOrAdmin,
  uploadController.processVideo
);

// Get video processing status
router.get('/videos/:fileId/status',
  uploadController.getVideoProcessingStatus
);

// Generate thumbnail from video
router.post('/videos/:fileId/thumbnail',
  requireInstructorOrAdmin,
...
router.post('/chunked/init',
  uploadController.initChunkedUpload
);

router.post('/chunked/:uploadId/chunk/:chunkNumber',
  upload.single('chunk'),
  uploadController.uploadChunk
);

router.post('/chunked/:uploadId/complete',
  uploadController.completeChunkedUpload
);

router.delete('/chunked/:uploadId',
  uploadController.cancelChunkedUpload
);
