...
class InstructorController extends BaseController {
  ...
  async getReviews(req: any, res: any) {
    try {
      ...
      const [reviews]: any = await this.getDatabase().query(`
        SELECT 
          r.*,
  ...
  async getRevenue(req: any, res: any) {
    try {
      const userId = req.user?.id;
      ...
      // Get revenue data
      const [revenue]: any = await this.getDatabase().query(`
        SELECT 
      ...
      // Get total revenue
      const [totals]: any = await this.getDatabase().query(`
        SELECT 
