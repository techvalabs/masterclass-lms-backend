...
interface CourseAnalysis {
  ...
  totalVideos: number;
  averageLessonLength: number; // in minutes
...
export class VideoAnalyzer {
   ...
   */
  static async analyzeCourseContent(
    modules: Array<{
      id: string;
      lessons: Array<{
        id: string;
        videoPath?: string;
        estimatedDuration?: number; // fallback if video not available
      }>;
    }>
    ...
    const pricingSuggestion = this.calculatePricingSuggestion(
      totalHours,
      totalVideos,
   ...
   */
  static calculatePricingSuggestion(
    totalHours: number,
    totalVideos: number,
    moduleCount: number,
    difficulty: 'beginner' | 'intermediate' | 'advanced'
  ): CoursePricingSuggestion {
