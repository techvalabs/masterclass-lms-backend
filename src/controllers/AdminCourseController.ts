...
export class AdminCourseController {
  ...
  createModule = async (req: Request, res: Response) => {
    try {
      ...
      const result = await db.query(
        ...
        [courseId, title, description || '', display_order || 1]
      );
  ...
  uploadLessonVideo = async (req: Request, res: Response) => {
    try {
      ...
      await db.query(
        `UPDATE course_lessons SET video_url = ? WHERE id = ?`,
        [videoPath, lessonId]
      );
  ...
  togglePublication = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
