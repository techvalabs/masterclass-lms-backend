...
export class AdminDashboardController {
   ...
   */
  getDashboardStats = async (req: AdminDashboardRequest, res: Response) => {
    try {
   ...
   */
  getRecentUsers = async (req: AdminDashboardRequest, res: Response) => {
    try {
      ...
      const [users]: any = await this.getDatabase().query(`
        SELECT 
          u.id,
          u.name,
          u.email,
   ...
   */
  getRecentCourses = async (req: AdminDashboardRequest, res: Response) => {
    try {
      const { limit = 5 } = req.query;
      ...
      // MySQL doesn't support parameterized LIMIT in prepared statements, so we use query() instead
      const limitNum = Math.min(Math.max(1, Number(limit)), 100); // Ensure limit is between 1 and 100
      const [courses]: any = await this.getDatabase().query(`
        SELECT 
          c.id,
          c.title,
          c.slug,
   ...
   */
  getAnalytics = async (req: AdminDashboardRequest, res: Response) => {
    try {
      const { period = '30d', startDate, endDate } = req.query as AnalyticsFilters;
      ...
      const [userRegistrations]: any = await this.getDatabase().execute(`
        SELECT 
      ...
      const [courseCreations]: any = await this.getDatabase().execute(`
        SELECT 
          DATE(created_at) as date,
          COUNT(*) as count
        FROM courses
      ...
      // Get overview stats
      const [overviewStats]: any = await this.getDatabase().execute(`
        SELECT 
      ...
      const [previousStats]: any = await this.getDatabase().execute(`
        SELECT 
      ...
      const analyticsData = {
        ...
        overview: {
          totalRevenue: {
            ...
            change: calculateChange(
              parseFloat(overviewStats[0].totalRevenue || 0),
              parseFloat(previousStats[0].totalRevenue || 0)
            )
          },
          totalUsers: {
            ...
            change: calculateChange(
              overviewStats[0].totalUsers || 0,
              previousStats[0].totalUsers || 0
            )
          },
          activeStudents: {
            ...
            change: calculateChange(
              overviewStats[0].activeStudents || 0,
              previousStats[0].activeStudents || 0
            )
          },
          totalCourses: {
            ...
            change: calculateChange(
              overviewStats[0].totalCourses || 0,
              previousStats[0].totalCourses || 0
            )
          },
          totalEnrollments: {
            ...
            change: calculateChange(
              overviewStats[0].totalEnrollments || 0,
              previousStats[0].totalEnrollments || 0
            )
          },
          courseCompletions: {
        ...
        userGrowth: userRegistrations.map((row: any) => ({
          date: row.date,
          newUsers: row.count
        })),
      ...
      // Get top courses
      try {
        const [topCoursesData]: any = await this.getDatabase().execute(`
          SELECT 
            c.id,
            c.title,
        ...
        analyticsData.topCourses = topCoursesData.map((course: any) => ({
          id: course.id,
          title: course.title,
