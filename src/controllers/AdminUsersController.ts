...
import { ApiResponse, User, PaginationQuery } from '../types/index.js';
...
interface BulkUserAction {
  action: 'activate' | 'deactivate' | 'delete' | 'verify' | 'change_role';
  userIds: number[];
  roleId?: number;
}

interface BulkEmailRequest {
  recipients: 'all' | 'role' | 'specific';
  userIds?: number[];
  roleId?: number;
  subject: string;
  message: string;
...
export class AdminUsersController {
   ...
   * GET /api/admin/users
   */
  getUsers = async (req: AdminUsersRequest, res: Response) => {
    try {
   ...
   */
  getUserById = async (req: AdminUsersRequest, res: Response) => {
    try {
   ...
   */
  updateUser = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
      ...
      if (email !== undefined) {
        ...
        const [emailCheck]: any = await this.db!.execute(
      ...
      if (roleId !== undefined) {
        // Verify role exists
        const [roles]: any = await this.db!.execute(
   ...
   */
  deleteUser = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
      ...
      const [enrollmentCheck]: any = await db.execute(
        'SELECT COUNT(*) as count FROM enrollments WHERE user_id = ? AND is_active = 1',
        [userId]
      );
      ...
      const [instructorCheck]: any = await db.execute(
        'SELECT COUNT(*) as count FROM courses c JOIN instructors i ON c.instructor_id = i.id WHERE i.user_id = ?',
        [userId]
      );
      ...
      try {
        ...
        try {
          await connection.execute(
            ...
            [req.user.id, 'user_deleted', 'user', userId, `Deleted user: ${user.name} (${user.email})`]
          );
        } catch (err: any) {
          // Skip if table doesn't exist
          if (err.code !== 'ER_NO_SUCH_TABLE') {
   ...
   */
  changeUserRole = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
   ...
   */
  toggleUserStatus = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
   ...
   */
  sendBulkEmail = async (req: AdminUsersRequest, res: Response) => {
    try {
      const { recipients, userIds, roleId, subject, message, template }: BulkEmailRequest = req.body;
   ...
   */
  bulkAction = async (req: AdminUsersRequest, res: Response) => {
    try {
      const { action, userIds, roleId }: BulkUserAction = req.body;
      ...
      switch (action) {
        ...
        case 'delete':
          ...
          const usersWithCourses = instructorCheck.filter((u: any) => u.course_count > 0);
          if (usersWithCourses.length > 0) {
            throw new ValidationError(`Cannot delete users who have created courses: ${usersWithCourses.map((u: any) => u.name).join(', ')}`);
          }
   ...
   */
  getUserStatistics = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
   ...
   */
  getUserActivity = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
      ...
      const allActivities = [
        ...enrollments.map((e: any, i: number) => ({ id: `enroll-${i}`, ...e })),
        ...completions.map((c: any, i: number) => ({ id: `complete-${i}`, ...c })),
        ...reviews.map((r: any, i: number) => ({ id: `review-${i}`, ...r, metadata: { rating: r.rating } })),
        ...purchases.map((p: any, i: number) => ({ id: `purchase-${i}`, ...p, metadata: { amount: p.amount } }))
   ...
   */
  sendEmailToUser = async (req: AdminUsersRequest, res: Response) => {
    try {
      const userId = parseInt(req.params.id);
   ...
   * Create a new user
   */
  createUser = async (req: AdminUsersRequest, res: Response) => {
    try {
      if (!this.db) {
