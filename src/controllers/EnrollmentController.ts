...
export class EnrollmentController extends BaseController {
  ...
  async createEnrollment(req: Request, res: Response) {
    ...
    try {
      ...
      const [existingEnrollment]: any = await db.execute(
      ...
      // Create enrollment
      const [enrollmentResult]: any = await db.execute(
      ...
      await db.execute(
        ...
        [userId, course_id, enrollmentResult.insertId]
      );
  ...
  async getMyEnrollments(req: Request, res: Response) {
    try {
      ...
      const [enrollments]: any = await this.getDatabase().query(
        `SELECT 
          e.*,
          c.id as course_id,
          c.title,
      ...
      const [countResult]: any = await this.getDatabase().execute(
        `SELECT COUNT(*) as total 
         FROM enrollments 
         WHERE user_id = ? ${status !== 'all' ? 'AND status = ?' : ''}`,
        status !== 'all' ? [userId, status] : [userId]
      );
  ...
  async getCourseEnrollment(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      const { courseId } = req.params;

      const [enrollment]: any = await this.getDatabase().execute(
        `SELECT 
          e.*,
          cp.progress_percentage,
          cp.completed_lessons,
      ...
      // Get lesson progress
      const [lessonProgress]: any = await this.getDatabase().execute(
        `SELECT 
          lp.*,
  ...
  async updateLessonProgress(req: Request, res: Response) {
    try {
      ...
      // Verify enrollment
      const [lesson]: any = await this.getDatabase().execute(
      ...
      try {
        ...
        await db.execute(
          `INSERT INTO lesson_progress 
           ...
           notes = VALUES(notes),
           last_watched_at = NOW()`,
          [userId, lessonId, videoProgress, isCompleted, timeSpent, notes]
        );

        // Update course progress
        const [totalLessons]: any = await db.execute(
          'SELECT COUNT(*) as total FROM course_lessons WHERE course_id = ?',
          [courseId]
        );

        const [completedLessons]: any = await db.execute(
          `SELECT COUNT(*) as completed 
           FROM lesson_progress lp
           JOIN course_lessons cl ON lp.lesson_id = cl.id
           WHERE lp.user_id = ? AND cl.course_id = ? AND lp.is_completed = 1`,
          [userId, courseId]
        );
  ...
  async submitQuizAttempt(req: Request, res: Response) {
    try {
      ...
      const [quiz]: any = await this.getDatabase().execute(
        `SELECT 
          q.*,
          cl.course_id
         FROM quizzes q
      ...
      // Get quiz questions
      const [questions]: any = await this.getDatabase().execute(
  ...
  async getCertificate(req: Request, res: Response) {
    try {
      ...
      const [certificate]: any = await this.getDatabase().execute(
