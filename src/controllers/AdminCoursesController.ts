...
import { ApiResponse, Course, Module, Lesson, PaginationQuery } from '../types/index.js';
...
interface BulkActionRequest {
  action: 'delete' | 'archive' | 'publish' | 'unpublish' | 'feature' | 'unfeature';
  courseIds: number[];
}
...
export class AdminCoursesController {
   ...
   */
  getCourses = async (req: AdminCoursesRequest, res: Response) => {
    try {
      ...
      const [countResult]: any = await this.getDatabase().query(countQuery);
      ...
      const [courses]: any = await this.getDatabase().query(coursesQuery, params);
   ...
   */
  getCourseById = async (req: AdminCoursesRequest, res: Response) => {
    try {
      if (!this.checkDatabaseConnection()) {
      ...
      const [courseResult]: any = await this.getDatabase().execute(courseQuery, [courseId]);
      ...
      const [lessons]: any = await this.getDatabase().execute(lessonsQuery, [courseId]);
      ...
      const modulesWithLessons = [{
        ...
        sort_order: 1,
        duration_seconds: lessons.reduce((total: number, lesson: any) => total + (lesson.duration_minutes * 60), 0),
   ...
   */
  createCourse = async (req: AdminCoursesRequest, res: Response) => {
    ...
    try {
      ...
      const [existingSlugs]: any = await this.getDatabase().execute(
      ...
      const [instructorCheck]: any = await this.getDatabase().execute(
      ...
      if (instructorCheck.length === 0) {
        ...
        const [instructorResult]: any = await this.getDatabase().execute(
          `INSERT INTO instructors (user_id, experience, status, created_at, updated_at) 
           VALUES (?, ?, ?, NOW(), NOW())`,
          [userId, 'System Administrator with LMS management experience', 'approved']
        );
      ...
      const [result]: any = await this.getDatabase().execute(insertQuery, [
        title,
        finalSlug,
        description,
      ...
      await this.getDatabase().execute(
        ...
        [req.user.id, 'course_created', 'course', courseId, `Created course: ${title}`]
      );
        ...
        for (const module of modules) {
          ...
          if (module.lessons && Array.isArray(module.lessons)) {
            for (const lesson of module.lessons) {
              ...
              // Insert lesson
              const [lessonResult]: any = await this.getDatabase().execute(
                `INSERT INTO course_lessons (course_id, title, slug, description, 
   ...
   */
  updateCourse = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
      ...
      const [existingCourse]: any = await this.getDatabase().execute(
      ...
      if (instructorId !== undefined) {
        ...
        const [instructors]: any = await this.getDatabase().execute(
      ...
      await this.getDatabase().execute(
        'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        [req.user.id, 'course_updated', 'course', courseId, `Updated course: ${title || existingCourse[0].title}`]
      );
   ...
   */
  deleteCourse = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
      ...
      try {
        ...
        await this.getDatabase().execute(
          'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
          [req.user.id, 'course_deleted', 'course', courseId, `Deleted course: ${course.title}`]
        );
   ...
   */
  togglePublishStatus = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
      ...
      // Check if course exists
      const [existingCourse]: any = await this.getDatabase().execute(
        'SELECT id, title, is_published FROM courses WHERE id = ?',
        [courseId]
      );
      ...
      await this.getDatabase().execute(
        'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        [req.user.id, newStatus ? 'course_published' : 'course_unpublished', 'course', courseId, `${newStatus ? 'Published' : 'Unpublished'} course: ${course.title}`]
      );
   ...
   */
  duplicateCourse = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
      ...
      const [lessons]: any = await this.getDatabase().execute(
        'SELECT * FROM course_lessons WHERE course_id = ?',
        [courseId]
      );
      ...
      await this.getDatabase().execute(
        'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        [req.user.id, 'course_duplicated', 'course', newCourseId, `Duplicated course: ${course.title} -> ${newTitle}`]
      );
   ...
   */
  approveCourse = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
      ...
      await this.getDatabase().execute(
        'UPDATE courses SET is_published = 1, publish_date = NOW(), updated_at = NOW() WHERE id = ?',
        [courseId]
      );
      ...
      await this.getDatabase().execute(
        'INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())',
        [req.user.id, 'course_approved', 'course', courseId, `Approved and published course: ${course.title}`]
      );
   ...
   */
  rejectCourse = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const courseId = parseInt(req.params.id);
   ...
   */
  bulkAction = async (req: AdminCoursesRequest, res: Response) => {
    try {
      const { action, courseIds }: BulkActionRequest = req.body;
      ...
      const [existingCourses]: any = await this.getDatabase().execute(
      ...
      switch (action) {
        case 'delete':
          ...
          const [enrollmentCheck]: any = await this.getDatabase().execute(
            `SELECT course_id, COUNT(*) as count FROM enrollments WHERE course_id IN (${placeholders}) AND is_active = 1 GROUP BY course_id`,
            courseIds
          );
      ...
      // Log bulk activity
      const courseNames = existingCourses.map((c: any) => c.title).join(', ');
      await this.getDatabase().execute(
