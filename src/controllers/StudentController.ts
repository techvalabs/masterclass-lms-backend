...
export class StudentController extends BaseController {
  ...
  async getCourseContent(req: Request, res: Response) {
    try {
      ...
      const [enrollment]: any = await this.getDatabase().query(
        'SELECT * FROM enrollments WHERE user_id = ? AND course_id = ? AND is_active = 1',
        [userId, courseId]
      );
      ...
      // Get modules
      const [modules]: any = await this.getDatabase().query(`
        SELECT * FROM modules 
      ...
      for (const module of modules) {
        const [lessons]: any = await this.getDatabase().query(`
          SELECT 
            l.*,
  ...
  async updateLessonProgress(req: Request, res: Response) {
    try {
      ...
      // Get lesson details
      const [lesson]: any = await this.getDatabase().query(
  ...
  private async updateCourseProgress(userId: number, courseId: number) {
    try {
      ...
      await this.getDatabase().query(
        'UPDATE enrollments SET progress_percentage = ?, last_accessed = NOW() WHERE user_id = ? AND course_id = ?',
  ...
  async getDashboard(req: Request, res: Response) {
    try {
      ...
      // Get recent activity
      const [activity]: any = await this.getDatabase().query(`
        SELECT 
