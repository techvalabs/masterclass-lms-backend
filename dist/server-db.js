...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        // Insert new user
        const [result] = await db.execute(`INSERT INTO users (name, email, password_hash, role_id, email_verified, created_at, updated_at) 
       VALUES (?, ?, ?, ?, 1, NOW(), NOW())`, [name, email, hashedPassword, roleId]);
        const userId = result.insertId;
        // Generate tokens
        const { accessToken, refreshToken } = generateTokens(userId, email, role);
        // Get the created user
        const [users] = await db.execute(`SELECT u.id, u.name, u.email, r.name as role, u.avatar, u.created_at 
       FROM users u 
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        // Get user with role
        const [users] = await db.execute(`SELECT u.id, u.name, u.email, u.password_hash, r.name as role, u.avatar 
       FROM users u 
       JOIN roles r ON u.role_id = r.id 
