...
export class VideoUploadController extends BaseController {
        ...
        try {
            ...
            upload(req, res, async (err) => {
                ...
                try {
                    ...
                    // Save to database
                    const [result] = await this.getDatabase().execute(`INSERT INTO video_uploads 
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`, [
                    ...
                    if (lessonId) {
                        await this.getDatabase().execute(`UPDATE course_lessons 
    ...
    generateThumbnail(videoPath, outputPath, timeInSeconds) {
    ...
    async processVideoQualities(videoPath, filename) {
        const qualities = [
    ...
    processVideo(inputPath, outputPath, quality) {
        return new Promise((resolve, reject) => {
            ffmpeg(inputPath)
                .outputOptions([
    ...
    // Generate HLS playlist for adaptive streaming
    generateHLS(videoPath, filename) {
    ...
    async getVideoStatus(req, res) {
        try {
            ...
            const [video] = await this.getDatabase().execute('SELECT * FROM video_uploads WHERE id = ?', [videoId]);
            if (video.length === 0) {
    ...
    async deleteVideo(req, res) {
        try {
            ...
            await this.getDatabase().execute('DELETE FROM video_uploads WHERE id = ?', [videoId]);
            res.json({
                success: true,
...
//# sourceMappingURL=VideoUploadController.js.map