...
export class MockPaymentController extends BaseController {
    ...
    async createMockCheckout(req, res) {
        try {
            ...
            try {
                ...
                await db.execute(`INSERT INTO payment_transactions 
           ...
           VALUES (?, ?, 0, NOW(), NOW())
           ON DUPLICATE KEY UPDATE last_accessed_at = NOW()`, [userId, courseId]);
    ...
    async getMockPaymentStatus(req, res) {
        try {
            const { transactionId } = req.params;
            const [transactions] = await this.getDatabase().execute(`SELECT 
          pt.*,
    ...
    async enrollFree(req, res) {
        try {
            const { courseId } = req.body;
            const userId = req.userId;
            // Check if course exists and is free
            const [courses] = await this.getDatabase().execute('SELECT * FROM courses WHERE id = ? AND (price = 0 OR price IS NULL)', [courseId]);
            if (courses.length === 0) {
                return res.status(404).json({
                    success: false,
            ...
            try {
                ...
                await db.execute(`INSERT INTO enrollments 
           (user_id, course_id, status, payment_status, enrolled_at, created_at) 
           VALUES (?, ?, 'active', 'free', NOW(), NOW())`, [userId, courseId]);
                // Update course stats
                await db.execute(`UPDATE courses 
           SET total_students = COALESCE(total_students, 0) + 1 
           WHERE id = ?`, [courseId]);
...
//# sourceMappingURL=MockPaymentController.js.map