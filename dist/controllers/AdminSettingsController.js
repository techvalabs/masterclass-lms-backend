...
export class AdminSettingsController {
        ...
        try {
            if (!this.checkDatabaseConnection()) {
                ...
                if (category) {
                    ...
                    filteredSettings = {};
                    if (realisticSettings[category]) {
                        filteredSettings[category] = realisticSettings[category];
                    }
                }
                return res.json({
                    success: true,
            ...
            const settings = {
                platform: platformConfig[0] || {},
                courses: courseSettings[0] || {},
                security: securitySettings[0] || {},
                registration: registrationSettings[0] || {},
                seo: seoSettings[0] || {},
                backup: backupSettings[0] || {}
            };
    ...
    getSystemInfo = async (req, res) => {
        try {
            ...
            const [dbStats] = await this.db.execute(`
        SELECT 
      ...
      `);
            // Get database size
            const [dbSize] = await this.db.execute(`
        SELECT 
      ...
      `);
            ...
            const [recentActivities] = await this.db.execute(`
    ...
    createBackup = async (req, res) => {
        try {
            const { name, type = 'full', tables, compression = 'gzip' } = req.body;
            ...
            const [result] = await this.db.execute(`INSERT INTO backup_logs (
          backup_name, backup_type, file_path, file_size, compression, status,
          started_at, created_by, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, NOW())`, [backupName, type, '', 0, compression, 'running', req.user.id]);
    ...
    getBackupStatus = async (req, res) => {
        try {
            ...
            const [backups] = await this.db.execute('SELECT * FROM backup_logs WHERE id = ?', [backupId]);
            if (backups.length === 0) {
    ...
    getBackups = async (req, res) => {
        try {
            ...
            // Count total records
            const [countResult] = await this.db.execute(`SELECT COUNT(*) as total FROM backup_logs ${whereClause}`, params);
            const total = countResult[0].total;
            // Calculate pagination
            const offset = (page - 1) * limit;
            const totalPages = Math.ceil(total / limit);
            // Get backups
            const [backups] = await this.db.execute(`SELECT 
          bl.*,
    ...
    deleteBackup = async (req, res) => {
        try {
            ...
            // Get backup details
            const [backups] = await this.db.execute('SELECT id, backup_name, file_path, status FROM backup_logs WHERE id = ?', [backupId]);
            if (backups.length === 0) {
                throw new NotFoundError('Backup not found');
            }
            const backup = backups[0];
            ...
            // Log activity
            await this.db.execute('INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())', [req.user.id, 'backup_deleted', 'backup', backupId, `Deleted backup: ${backup.backup_name}`]);
            res.json({
                success: true,
    ...
    clearCache = async (req, res) => {
        try {
            ...
            await this.db.execute('INSERT INTO activity_logs (user_id, action, entity_type, description, created_at) VALUES (?, ?, ?, ?, NOW())', [req.user.id, 'cache_cleared', 'system', 'Application cache cleared']);
            res.json({
                success: true,
    ...
    // Helper methods
    async performBackupAsync(backupId, backupName, type, tables, compression, backupDir) {
        try {
            let filePath = '';
            let fileSize = 0;
            const metadata = {
            ...
            await this.db.execute(`UPDATE backup_logs SET 
          file_path = ?, 
          file_size = ?, 
          status = 'completed', 
          completed_at = NOW(),
    ...
    async updateBackupStatus(backupId, status, errorMessage) {
        try {
            await this.db.execute('UPDATE backup_logs SET status = ?, error_message = ?, completed_at = NOW() WHERE id = ?', [status, errorMessage || null, backupId]);
        }
        catch (error) {
     ...
     */
    async logSettingsChange(category, changes, userId, ipAddress) {
        try {
