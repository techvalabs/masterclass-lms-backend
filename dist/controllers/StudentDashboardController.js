...
export class StudentDashboardController extends BaseController {
    ...
    async getDashboardStats(req, res) {
      ...
      `, [userId, userId, userId, userId, userId, userId, userId, userId]);
            ...
            const [streakData] = await this.getDatabase().query(`
        SELECT 
      ...
      `, [userId]);
            ...
            if (lastActive === today || lastActive === yesterday) {
                ...
                const [streakDays] = await this.getDatabase().query(`
    ...
    async getRecentActivity(req, res) {
        try {
            ...
            const [activities] = await this.getDatabase().query(`
        (SELECT 
    ...
    async getLeaderboard(req, res) {
        try {
            ...
            // Get top students
            const [leaderboard] = await this.getDatabase().query(`
        SELECT 
          u.id as userId,
          u.name as userName,
      ...
      `);
            ...
            const [userPosition] = await this.getDatabase().query(`
        SELECT 
          COUNT(*) + 1 as position
        FROM (
          SELECT 
     ...
     */
    async toggleCourseFavorite(req, res) {
        try {
            const userId = req.user?.id;
            const { courseId } = req.params;
            // Check if already favorited
            const [existing] = await this.getDatabase().query('SELECT * FROM user_favorites WHERE user_id = ? AND course_id = ?', [userId, courseId]);
            let isFavorite = false;
            if (existing.length > 0) {
                // Remove favorite
                await this.getDatabase().query('DELETE FROM user_favorites WHERE user_id = ? AND course_id = ?', [userId, courseId]);
            }
            else {
                // Add favorite
                await this.getDatabase().query('INSERT INTO user_favorites (user_id, course_id, created_at) VALUES (?, ?, NOW())', [userId, courseId]);
                isFavorite = true;
            }
            return res.json({
                success: true,
    ...
    async getRecommendedCourses(req, res) {
        try {
            ...
            const [userCategories] = await this.getDatabase().query(`
      ...
      `;
            ...
            query += ` GROUP BY c.id
                 ...
                 LIMIT 6`;
            const [recommendations] = await this.getDatabase().query(query, params);
            return res.json({
                success: true,
     ...
     */
    async getLearningStreaks(req, res) {
        try {
            const userId = req.user?.id;
            // Get last 30 days of activity
            const [streakHistory] = await this.getDatabase().query(`
      ...
      `, [userId]);
            ...
            const lastActivityDate = streakHistory.find((d) => d.hasActivity)?.date || null;
            return res.json({
                success: true,
                data: {
                    currentStreak,
    ...
    async getAchievements(req, res) {
        try {
            ...
            const [achievements] = await this.getDatabase().query(`
        SELECT 
          a.id,
          a.name,
          a.description,
