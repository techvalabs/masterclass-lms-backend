export default class AdminLessonsController {
    ...
    async getLessonById(req, res) {
      ...
      `, [lessonId]);
            ...
            lesson.quiz = quizzes.length > 0 ? quizzes[0] : null;
            res.json({
                success: true,
                data: lesson
            });
    ...
    async createLesson(req, res) {
        const { courseId } = req.params;
        const { title, description, video_url, video_duration, content, is_preview, is_published, sort_order } = req.body;
        try {
            const db = this.getDatabase();
    ...
    async updateLesson(req, res) {
        ...
        try {
            ...
            const fields = Object.keys(updates).filter(key => !['id', 'course_id', 'created_at'].includes(key));
            if (fields.length === 0) {
    ...
    async createQuiz(req, res) {
        const { lessonId } = req.params;
        const { title, description, pass_percentage, time_limit, max_attempts, questions } = req.body;
        try {
            const db = this.getDatabase();
            // Start transaction
            await db.query('START TRANSACTION');
...
//# sourceMappingURL=AdminLessonsController.js.map