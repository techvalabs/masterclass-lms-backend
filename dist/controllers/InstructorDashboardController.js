...
export class InstructorDashboardController extends BaseController {
    ...
    async getCourseMetrics(req, res) {
        try {
            ...
            const [course] = await this.getDatabase().query('SELECT * FROM courses WHERE id = ? AND instructor_id = ?', [courseId, instructorId]);
            ...
            const [metrics] = await this.getDatabase().query(`
        SELECT 
      ...
      `, [courseId]);
            ...
            const [engagement] = await this.getDatabase().query(`
        SELECT 
    ...
    async getStudentProgress(req, res) {
      ...
      `;
            ...
            const [progress] = await this.getDatabase().query(query, params);
            return res.json({
                success: true,
                data: progress
            });
...
//# sourceMappingURL=InstructorDashboardController.js.map