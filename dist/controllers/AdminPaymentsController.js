...
export class AdminPaymentsController {
    ...
    getTransactions = async (req, res) => {
      ...
      `;
        ...
        LIMIT ? OFFSET ?
      `;
            const [transactions] = await this.db.execute(transactionsQuery, [...params, limit, offset]);
            const response = {
                success: true,
    ...
    getRefunds = async (req, res) => {
        try {
            if (!this.checkDatabaseConnection()) {
                ...
                if (search) {
                    const searchLower = search.toLowerCase();
                    filteredRefunds = filteredRefunds.filter(r => r.user.name.toLowerCase().includes(searchLower) ||
      ...
      `;
        ...
        LIMIT ? OFFSET ?
      `;
            const [refunds] = await this.db.execute(refundsQuery, [...params, limit, offset]);
            const response = {
                success: true,
    ...
    processRefund = async (req, res) => {
        try {
            ...
            const [transactions] = await this.db.execute('SELECT * FROM payment_transactions WHERE id = ? AND status = ?', [transactionId, 'completed']);
            if (transactions.length === 0) {
                throw new NotFoundError('Transaction not found or not eligible for refund');
            }
            const transaction = transactions[0];
            // Check if refund amount is valid
            const [existingRefunds] = await this.db.execute('SELECT COALESCE(SUM(amount), 0) as total_refunded FROM refunds WHERE transaction_id = ? AND status = ?', [transactionId, 'completed']);
            ...
            try {
                ...
                const [refundResult] = await this.db.execute(`INSERT INTO refunds (
            ...
            status, processed_by, processed_at, created_at, updated_at
                ...
                if (newTotalRefunded >= parseFloat(transaction.amount)) {
                    await this.db.execute('UPDATE payment_transactions SET status = ?, updated_at = NOW() WHERE id = ?', ['refunded', transactionId]);
                }
                else {
                    await this.db.execute('UPDATE payment_transactions SET status = ?, updated_at = NOW() WHERE id = ?', ['partially_refunded', transactionId]);
                }
                // Update enrollment if course-related
                if (transaction.course_id) {
                    await this.db.execute('UPDATE enrollments SET payment_status = ?, updated_at = NOW() WHERE user_id = ? AND course_id = ?', [newTotalRefunded >= parseFloat(transaction.amount) ? 'refunded' : 'partially_refunded', transaction.user_id, transaction.course_id]);
                }
                // Log activity
    ...
    getCoupons = async (req, res) => {
        try {
            if (!this.checkDatabaseConnection()) {
                ...
                if (status === 'active') {
                    filteredCoupons = filteredCoupons.filter(c => c.isActive && (!c.expiresAt || new Date(c.expiresAt) > new Date()));
                }
                ...
                if (search) {
                    const searchLower = search.toLowerCase();
                    filteredCoupons = filteredCoupons.filter(c => c.code.toLowerCase().includes(searchLower) ||
      ...
      `;
        ...
        LIMIT ? OFFSET ?
      `;
            const [coupons] = await this.db.execute(couponsQuery, [...params, limit, offset]);
            const response = {
                success: true,
    ...
    createCoupon = async (req, res) => {
        try {
            const { code, name, description, type, value, minimumAmount, maximumDiscount, usageLimit, usageLimitPerUser = 1, applicableCourses, applicableCategories, excludeCourses, excludeCategories, startsAt, expiresAt, isActive = true } = req.body;
            // Validation
            ...
            const [existingCoupons] = await this.db.execute('SELECT id FROM coupons WHERE code = ?', [code.toUpperCase()]);
      ...
      `;
            ...
            // Log activity
            await this.db.execute('INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())', [req.user.id, 'coupon_created', 'coupon', couponId, `Created coupon: ${code}`]);
            res.status(201).json({
                success: true,
    ...
    updateCoupon = async (req, res) => {
        try {
            ...
            const [existingCoupon] = await this.db.execute('SELECT id, code, used_count FROM coupons WHERE id = ?', [couponId]);
            if (existingCoupon.length === 0) {
                throw new NotFoundError('Coupon not found');
            }
            const coupon = existingCoupon[0];
            const { name, description, value, minimumAmount, maximumDiscount, usageLimit, usageLimitPerUser, applicableCourses, applicableCategories, excludeCourses, excludeCategories, startsAt, expiresAt, isActive } = req.body;
      ...
      `;
            ...
            // Log activity
            await this.db.execute('INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())', [req.user.id, 'coupon_updated', 'coupon', couponId, `Updated coupon: ${coupon.code}`]);
            res.json({
                success: true,
    ...
    deleteCoupon = async (req, res) => {
        try {
            ...
            // Log activity
            await this.db.execute('INSERT INTO activity_logs (user_id, action, entity_type, entity_id, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())', [req.user.id, 'coupon_deleted', 'coupon', couponId, `Deleted coupon: ${coupon.code}`]);
            res.json({
                success: true,
...
//# sourceMappingURL=AdminPaymentsController.js.map