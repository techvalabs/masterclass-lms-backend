...
export class CertificateController extends BaseController {
    ...
    async generateCertificate(req, res) {
        try {
            ...
            const [enrollment] = await this.getDatabase().query(`
        SELECT 
          e.*,
          cp.is_completed,
      ...
      `, [userId, courseId]);
            ...
            const [existingCert] = await this.getDatabase().query('SELECT * FROM certificates WHERE user_id = ? AND course_id = ?', [userId, courseId]);
            ...
            // Create certificate
            const [result] = await this.getDatabase().query(`
      ...
      `, [userId, courseId, certificateNumber, verificationCode]);
            ...
            const [certificate] = await this.getDatabase().query('SELECT * FROM certificates WHERE id = ?', [result.insertId]);
            // Update enrollment with certificate info
            await this.getDatabase().query('UPDATE enrollments SET certificate_issued_at = NOW() WHERE user_id = ? AND course_id = ?', [userId, courseId]);
            return res.json({
                success: true,
     ...
     */
    async getCertificateByCourse(req, res) {
        try {
            const { courseId } = req.params;
            const userId = req.user?.id;
            const [certificate] = await this.getDatabase().query(`
        SELECT 
          c.*,
    ...
    async getUserCertificates(req, res) {
        try {
            const userId = req.user?.id;
            const [certificates] = await this.getDatabase().query(`
        SELECT 
          c.*,
          co.title as course_name,
          co.thumbnail,
     ...
     */
    async formatCertificate(certificate) {
        const [courseData] = await this.getDatabase().query(`
      SELECT 
        c.title,
        c.duration_hours,
        i.user_id as instructor_user_id,
