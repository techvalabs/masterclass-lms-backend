...
router.post('/avatar', authenticate, upload.single('avatar'), async (req, res) => {
    try {
        ...
        await pool.execute('UPDATE users SET avatar = ?, updated_at = NOW() WHERE id = ?', [avatarUrl, userId]);
        res.json({
            success: true,
            data: {
                avatarUrl
...
router.post('/change-password', authenticate, async (req, res) => {
    try {
        ...
        // Update password
        await pool.execute('UPDATE users SET password_hash = ?, updated_at = NOW() WHERE id = ?', [newPasswordHash, userId]);
        res.json({
            success: true,
