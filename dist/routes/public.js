...
router.get('/course-preview/:id', async (req, res) => {
    try {
        ...
        if (authHeader && authHeader.startsWith('Bearer ')) {
            try {
                ...
                const [users] = await pool.execute('SELECT u.id, r.name as role FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = ?', [decoded.userId]);
    ...
    `, [courseId]);
      ...
      WHERE course_id = ?
    `, [courseId]);
        course.total_students = enrollments[0]?.enrollment_count || 0;
      ...
      WHERE course_id = ?
    `, [courseId]);
        course.total_reviews = reviews[0]?.review_count || 0;
        course.rating = reviews[0]?.average_rating || 0;
        res.json({
            success: true,
            data: course
        });
        ...
        for (const question of questions) {
            ...
            if (question.type === 'multiple') {
                ...
                isCorrect = correctArray.length === userArray.length &&
                    correctArray.every((ans) => userArray.includes(ans));
            }
            else {
