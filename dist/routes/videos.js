...
router.post('/upload', authorize(['admin', 'instructor']), videoController.uploadLessonVideo.bind(videoController));
// Get video status
router.get('/:videoId/status', videoController.getVideoStatus.bind(videoController));
// Delete video (admin/instructor only)
router.delete('/:videoId', authorize(['admin', 'instructor']), videoController.deleteVideo.bind(videoController));
