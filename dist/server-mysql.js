...
async function safeDbOperation(operation, fallbackData, errorMessage = 'Database operation failed') {
...
function generateTokens(userId, email, role) {
    const accessToken = jwt.sign({ userId, email, role, type: 'access' }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
    const refreshToken = jwt.sign({ userId, email, role, type: 'refresh' }, JWT_REFRESH_SECRET, { expiresIn: JWT_REFRESH_EXPIRES_IN });
...
app.get('/api/debug/users', async (req, res) => {
    try {
        const [users] = await db.execute(`SELECT u.id, u.name, u.email, u.role_id,
              r.name as role_name
       FROM users u 
...
app.get('/api/health', async (req, res) => {
    ...
    if (checkDatabaseConnection()) {
        const result = await safeDbOperation(async () => {
            ...
            const [tables] = await db.execute("SHOW TABLES LIKE 'users'");
            tableStatus = tables.length > 0 ? 'ready' : 'missing';
            return true;
        }, false, 'Health check database operation');
        if (!result) {
            dbStatus = 'error';
        }
    }
...
app.get('/api/auth/me', authenticateToken, async (req, res) => {
    try {
        const [users] = await db.execute(`SELECT u.id, u.name, u.email, 
...
app.get('/api/courses/featured', async (req, res) => {
    ...
    const courses = await safeDbOperation(async () => {
    ...
    }, mockCourses, 'Featured courses fetch');
    res.json({
        success: true,
        data: courses,
...
app.get('/api/courses/categories', async (req, res) => {
    try {
        const [categories] = await db.execute('SELECT name FROM categories WHERE is_active = 1 ORDER BY name');
