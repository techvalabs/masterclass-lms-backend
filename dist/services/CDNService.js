...
export class CDNService {
    ...
    async uploadToCloudflare(filePath, key, contentType) {
    ...
    async uploadToCloudflareStream(filePath, key) {
    ...
    // Upload to Bunny CDN
    async uploadToBunny(filePath, key, contentType) {
        ...
        try {
            const fileBuffer = await fs.readFile(filePath);
            const response = await fetch(`https://storage.bunnycdn.com/${storageZone}/${key}`, {
                method: 'PUT',
                headers: {
                    'AccessKey': apiKey,
    ...
    async handleLocalStorage(filePath, key) {
    ...
    async deleteFromCloudflare(key) {
        // Implementation for Cloudflare deletion
    }
    async deleteFromBunny(key) {
        const storageZone = process.env.BUNNY_STORAGE_ZONE;
        const apiKey = process.env.BUNNY_API_KEY;
        if (!storageZone || !apiKey)
            return;
        try {
            await fetch(`https://storage.bunnycdn.com/${storageZone}/${key}`, {
                method: 'DELETE',
                headers: {
                    'AccessKey': apiKey
                }
    ...
    async optimizeImage(imagePath, sizes = [480, 720, 1080, 1920]) {
    ...
    async generateHLSManifest(videoKey, qualities) {
        const manifest = `#EXTM3U
#EXT-X-VERSION:3
${qualities.map(q => `#EXT-X-STREAM-INF:BANDWIDTH=${q.bitrate},RESOLUTION=${q.width}x${q.height}
${this.getCDNUrl(q.key)}`).join('\n')}`;
