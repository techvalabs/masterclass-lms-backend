...
export const CommonValidation = {
    ...
    // Password
    password: Joi.string().min(8).max(128).pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])/).message('Password must contain at least one lowercase letter, one uppercase letter, one number, and one special character'),
    // Name
    ...
    date: Joi.date().iso(),
    // JSON field
    json: Joi.alternatives().try(Joi.string().custom((value, helpers) => {
        try {
...
export const QuizValidation = {
    ...
    submitQuizAttempt: {
        params: CommonValidation.id,
        body: Joi.object({
            answers: Joi.object().pattern(Joi.number().integer().positive(), Joi.alternatives().try(Joi.string().trim().max(1000), Joi.array().items(Joi.string().trim().max(255)), Joi.boolean())).required(),
...
export const validateUuidParam = (paramName) => {
    const schema = Joi.object({
        [paramName]: Joi.alternatives().try(Joi.string().guid({ version: 'uuidv4' }), Joi.number().integer().positive()).required()
    });
