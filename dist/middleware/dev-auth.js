 ...
 */
export async function devAuthenticate(req, res, next) {
    try {
        let token;
        ...
        const user = usersData.find((u) => u.id === payload.userId && u.is_active);
        if (!user) {
            return res.status(401).json({
                success: false,
 ...
 */
export function devRequireAdmin(req, res, next) {
    if (!req.user) {
        return res.status(401).json({
            success: false,
 ...
 */
export async function devAuthenticateAdmin(req, res, next) {
    try {
