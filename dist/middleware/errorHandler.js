import { AppError, parseDatabaseError, sendError } from '@/utils/errors.js';
...
export const handleValidationErrors = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        ...
        const apiError = new AppError('Validation failed', 400, 'VALIDATION_ERROR');
...
export const rateLimitHandler = (req, res, next) => {
    const error = new AppError('Too many requests from this IP, please try again later', 429, 'RATE_LIMIT_EXCEEDED');
...
export const multerErrorHandler = (error, req, res, next) => {
    if (error.code === 'LIMIT_FILE_SIZE') {
        const apiError = new AppError('File size too large', 400, 'FILE_TOO_LARGE');
        return next(apiError);
    }
    if (error.code === 'LIMIT_FILE_COUNT') {
        const apiError = new AppError('Too many files uploaded', 400, 'TOO_MANY_FILES');
        return next(apiError);
    }
    if (error.code === 'LIMIT_UNEXPECTED_FILE') {
        const apiError = new AppError('Unexpected file field', 400, 'UNEXPECTED_FILE');
        return next(apiError);
    }
    // Generic multer error
    const apiError = new AppError('File upload error', 400, 'FILE_UPLOAD_ERROR');
    next(apiError);
};
/**
...
export const jsonErrorHandler = (error, req, res, next) => {
    if (error instanceof SyntaxError && 'body' in error) {
        const apiError = new AppError('Invalid JSON in request body', 400, 'INVALID_JSON');
        return next(apiError);
    }
    next(error);
};
 ...
 */
export const shutdownHandler = (req, res, next) => {
    const error = new AppError('Server is shutting down', 503, 'SERVER_SHUTDOWN');
